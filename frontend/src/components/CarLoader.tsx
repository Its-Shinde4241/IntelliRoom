...
export default function CarLoader({ className = '' }: CarLoaderProps) {
    const containerRef = useRef<HTMLDivElement>(null);
    const roadStartRef = useRef<HTMLDivElement>(null);
    const roadEndRef = useRef<HTMLDivElement>(null);
    const centerLineStartRef = useRef<HTMLDivElement>(null);
    const centerLineEndRef = useRef<HTMLDivElement>(null);

    return (
        <div className={`car-loader-container ${className}`}>
            <div className="car-loader-content" ref={containerRef}>
                {/* Road reference points */}
                <div ref={roadStartRef} className="absolute left-0 top-1/2 -translate-y-1/2 w-px h-px opacity-0 pointer-events-none"></div>
                <div ref={roadEndRef} className="absolute right-0 top-1/2 -translate-y-1/2 w-px h-px opacity-0 pointer-events-none"></div>
                <div ref={centerLineStartRef} className="absolute left-0 top-1/2 -translate-y-1/2 w-px h-px opacity-0 pointer-events-none"></div>
                <div ref={centerLineEndRef} className="absolute right-0 top-1/2 -translate-y-1/2 w-px h-px opacity-0 pointer-events-none"></div>

                {/* Animated road lines using AnimatedBeam */}
                {/* Top road boundary - much slower and lengthy */}
                <AnimatedBeam
                    containerRef={containerRef}
                    fromRef={roadStartRef}
                    toRef={roadEndRef}
                    pathWidth={1}
                    pathOpacity={1}
                ...
                <div className="car-container">
                    ...
                    <div className="car-svg-container">
                        ...
                        <div className="smoke-container">
                            {[...Array(8)].map((_, i) => (
                                <div
                                    key={i}
                                    className={`smoke-particle smoke-${i % 3}`}
                                    style={{
                                        left: `${-i * 4}px`,
                                        top: `${Math.sin(i * 0.7) * 12}px`,
                    ...
                    {/* Loading text attached to back of car */}
                    <div className="loading-text-attached">
                        {/* Thread connecting car to text */}
                        <div className="loading-content">
                            <span className="text-accent-foreground font-bold animate-accordion-up">Loading</span>
                            {/* <div className="flex align-middle items-center justify-center gap-1 mt-1">
                                <div className="bg-black size-2 rounded-2xl ease-in animate-bounce"></div>
                                <div className="bg-black size-2 rounded-2xl ease-in-out animate-bounce"></div>
                                <div className="bg-black size-2 rounded-2xl ease-in animate-bounce"></div>
                            </div> */}
                        </div>
                    </div>
