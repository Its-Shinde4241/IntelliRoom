...
export default function RoomSettingsPopover({
  ...
  const handleUpdateName = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Failed to update room name : " + error);
    } finally {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      setIsSubmitting(true);
      await onUpdateRoomWithPassword(
        roomName.trim() || currentRoomName,
        newPassword,
      );
      toast.success("Room password updated successfully!");
      setOpen(false);
    } catch (error) {
      toast.error("Failed to update room password : " + error);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} modal={false}>
      ...
      <PopoverContent className="w-96" align="end" side="bottom" sideOffset={4}>
